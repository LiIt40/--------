...
# Так же видно, что пропусков по столбцам нет, но проверим это еще раз отдельно:
print(f'Общее количество пропусков:\n{df_all.isnull().sum()}')

#Проверим наличие дубликатов

print(f'количество дубликатов = {df_all.duplicated().sum()}')
# Данные очищены и готовы к дальнейшему использованию
...
with metrics:
    ...
    avg_check = total_sales/total_count_orders
...
with visuals:
    ...
    # Линейный график
    ax3.plot(
        count_orders_by_day['order_date'].sort_values(),
        count_orders_by_day['order_id'],
        marker = 'o'
    )
...
with conclusions:
    st.write(f'Основываясь на данных, полученных в результате анализа можно сделать следующие выводы:\n'
             ...
             f'4. Пик продаж приходится на четверг.'
             ' Так же видим на графике повышение количества продаж в выходные.\n'
             )
